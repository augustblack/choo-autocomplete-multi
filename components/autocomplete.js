...
module.exports = (app, {
  ...
  return (state, emit) => {
    ...
    const showMore = (state, emit) => {
      if ((state.page + 1) * state.perPage < state.total) { return html`<li class="suggestion" onmousedown=${e => { e.preventDefault(); emit(sym.fetchMore) }}>Show more....${state.loading ? 'loading' : ''}</li>` }
      return html``
    }
